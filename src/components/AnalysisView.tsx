 ...
 * 상장 이후 일별 종가를 받아 매수/매도 기준가 두 개로 압축해 보여 준다.
 */
function AnalysisView() {
    ...
    // 1) 조회 중 / 실패 — 그릴 값이 없으므로 안내로 대체한다
    if (technical.loading) {
        return (
            <div className={'analysis_view'}>
                <p className={'analysis_empty_page'}>시세를 불러와 기준가를 계산하는 중입니다…</p>
            </div>
        )
    }
    ...
    if (technical.error !== null || technical.data === null) {
        return (
            <div className={'analysis_view'}>
                <p className={'analysis_empty_page'}>
                    시세를 불러오지 못했습니다 — {technical.error ?? '데이터 없음'}
                </p>
            </div>
        )
    }
        ...
        <div className={'analysis_view'}>
            ...
            <section className={'verdict_card'}>
                ...
                <header className={'verdict_card_head'}>
                    <span className={'verdict_symbol'}>JEPQ</span>
                    <span className={'verdict_asof'}>기준 {report.asOf} · 종가</span>
                </header>
                ...
                <footer className={'verdict_card_foot'}>
                    ...
                    {/* 기다리는 전략이라 "이 문이 얼마나 드물게 열리는지"를 반드시 같이 알려 준다 */}
                    <p className={'verdict_basis_text'}>
                        최근 1년 중 매수 구간이 열린 날은 <b>{report.buyDaysLastYear}일</b>뿐이었습니다.
                        기다리는 동안 현금을 놀리면 못 받은 분배금이 진입가 이득을 넘어서므로,
